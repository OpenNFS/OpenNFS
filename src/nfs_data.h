...
struct POLYGONDATA {
    ...
    unsigned char flags;    // 00 normally, 20 at end of row, 10 two-sided (HS  // used for animated textures //AnimInfo
                            // (Length : Period AS LSB 3:HSB 5))
...
struct FCE {
    ...
    struct NFS4 {
        ...
        //    K : "H" (Headlights); "T" (Taillights); "B" (Brakelight); "R" (Reverse light); "P" (Direction indicator);
        //    "S" (Siren); C : "W" (White); "R" (Red); "B" (Blue); "O" (Orange); "Y" (Yellow) B : "Y" (Yes); "N" (No) F
        //    : "O" (Flashing at moment 1); "E" (Flashing at moment 2); "N" (No flashing) I : Number between 0 and 9
        //    with 0 being broken (normal max 5)
...
struct COLLISION_BLOCK {
    ...
    uint16_t unknown;    // The left and right border values indicate the two limits beyond which no car can go. This is
                         // the data used for delimitation between the road and scenery
    uint16_t leftBorder; // Formula to find the coordinates of the left-most point of the road is (left-most point) =
                         // (reference point) - 2.(left border).(right vector):  there is a factor of 2 between absolute
    uint16_t rightBorder; // 32-bit coordinates and the othe data in the record. Similarly, for the right-most point of
                          // the road, (right-most point) = (reference point) + 2.(right border).(right vector).
...
#pragma pack(push, 2)
        struct HEADER {
            ...
            uint32_t unknown[32]; // useless list with values, which increase by 0x4 (maybe global offset list, which is
                                  // needed for calculating the position of the blocks)
...
struct PS1 {
    ...
    struct PSH {
        ...
        struct IMAGE_HEADER {
            uint8_t imageType; // Image type: Observed values are 0x40, 0x42, 0x43, and 0xC0 The bottom 2 bits of the
                               // image type byte specify the bit depth of the image: 0 - 4-bit indexed colour 2 -
                               // 16-bit direct colour 3 - 24-bit direct colour
            uint8_t unknown[3];
            uint16_t width;
            uint16_t height;
            uint16_t unknown2[4];
...
#pragma pack(push, 2)
        struct HEADER {
            uint32_t padding;     // Possible value: 0x00, 0x01, 0x02
            uint16_t unknown[64]; // useless list with values, which increase by 0x4 (maybe global offset list, which is
                                  // needed for calculating the position of the blocks)
            uint64_t unknown2;    //  always 0x00
        };
        ...
        struct BLOCK_HEADER {
            uint32_t nVerts;
            uint32_t
                unknown1; // Block type? Changes how many padding bytes there are uint16_t[(unknown1 + extraPadByte)*2]
        ...
        struct POLY_3D {
            ...
            uint16_t vertex[3][4]; // Literally wtf, 3 groups of 4 numbers that look like the vert indexes. One set [1]
                                   // is usually 0,0,0,0 or 1,1,1,1
            char texName[4];
        };
