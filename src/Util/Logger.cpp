...
namespace OpenNFS {
    Logger::Logger() {
        ...
        auto defaultHandler = gMainLogger->addSink(std::make_unique<FileSink>("OpenNFS", LOG_FILE_PATH, ""),
                                                   &FileSink::fileWrite);
        ...
        auto coutSinkHandle = gMainLogger->
                addSink(std::make_unique<ColorCoutSink>(), &ColorCoutSink::ReceiveLogMessage);
        auto onScreenSinkHandle = gMainLogger->addSink(std::make_unique<OnScreenLogSink>(&onScreenLog),
                                                       &OnScreenLogSink::ReceiveLogMessage);
