...
namespace OpenNFS {
    Logger::Logger() {
        ...
        auto const defaultHandler{
            gMainLogger->addSink(std::make_unique<FileSink>("OpenNFS", LOG_FILE_PATH, ""), &FileSink::fileWrite)};
        ...
        auto onScreenSinkHandle{
            gMainLogger->addSink(std::make_unique<OnScreenLogSink>(&onScreenLog), &OnScreenLogSink::ReceiveLogMessage)};
