...
#define ASSERT(condition, message)                                                                                     \
    do {                                                                                                               \
        if (!(condition)) {                                                                                            \
            LOG(WARNING) << "Assertion `" #condition "` failed in " << __FILE__ << " line " << __LINE__ << ": "        \
                         << message;                                                                                   \
            LOG(WARNING)                                                                                               \
                << "Press ESC to terminate, and let me know on Discord! (if you're sure this isn't your own fault)";   \
            char c;                                                                                                    \
            while (true) {                                                                                             \
                c = std::getchar();                                                                                    \
                if (c == 27)                                                                                           \
...
struct OnScreenLogSink {
    ...
    explicit OnScreenLogSink(AppLog *targetLog) { log = targetLog; };
