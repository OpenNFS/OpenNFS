...
btBoxShape *genCollisionBox(std::vector<glm::vec3> model_vertices) {
    ...
    return new btBoxShape(btVector3((top_right.x - bottom_left.x) / 2, (top_right.y - bottom_left.y) / 2,
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            ...
            for (size_t v = 0; v < fv; v++) {
                ...
                norms.emplace_back(glm::vec3(attrib.normals[3 * idx.normal_index + 0],
        ...
        CarModel obj_mesh =
            CarModel(shapes[s].name + "_obj", verts, uvs, norms, indices, glm::vec3(0, 0, 0), 0.01f, 0.0f, 0.5f);
    ...
    for (uint32_t image_Idx = 0; image_Idx < pshHeader->nDirectories; ++image_Idx) {
        ...
        if (bitDepth == 0 || bitDepth == 1) {
            ...
            if (paletteHeader->paletteHeight != 1) {
                ...
                // paletteHeaders, then jump backwards by how offset 'height' is into paletteHeader to get proper
                                                    ...
                                                    for (; h > 0; h--) {
                                                        for (w = width; w > 0; w--) {
                                                            ...
                                                            *current_bits++ = rgba.rgbBlue;
                                                            if (rgba.rgbRed == 0 && rgba.rgbGreen == alphaColour &&
                                                                rgba.rgbBlue == 0) {
                                                                *current_bits++ = 0;
                                                            } else {
                                                                *current_bits++ = 255;
                                                    ...
                                                    for (; h > 0; h--) {
                                                        for (w = width; w > 0; w--) {
                                                            ...
                                                            *current_bits++ = pixel[0];
                                                            if (pixel[2] == 0 && pixel[1] == alphaColour &&
                                                                pixel[0] == 0) {
                                                                *current_bits++ = 0;
                                                            } else {
                                                                *current_bits++ = 255;
            ...
            if ((fread(data, size, 1, fp) == 1) && (fread(data_a, size_a, 1, fp_a) == 1)) {
                ...
                if (file_header->bfType == MAKEuint16_t('B', 'M')) {
                    if (file_header->bfSize == (uint32_t)size) {
                        CP_BITMAPINFO *info =
                            (CP_BITMAPINFO *)(data +
                                              sizeof(CP_BITMAPFILEHEADER)); // we only handle uncompressed bitmaps
                        CP_BITMAPINFO *info_a =
                            (CP_BITMAPINFO *)(data_a +
                                              sizeof(CP_BITMAPFILEHEADER)); // we only handle uncompressed bitmaps
                        if (info->bmiHeader.biCompression == CP_BI_RGB) {
                            width = info->bmiHeader.biWidth;
                            *width_ = width;
