...
Car::Car(std::vector<CarModel> car_meshes, NFSVer nfs_version, std::string car_name, GLuint car_textureArrayID)
    : Car(car_meshes, nfs_version, car_name) {
    textureArrayID = car_textureArrayID;
    multitexturedCarModel = true;
}

Car::Car(uint16_t populationID, std::vector<CarModel> car_meshes, NFSVer nfs_version, std::string car_name,
         RaceNet carNet)
    : Car(car_meshes, nfs_version, car_name) {
...
Car::Car(std::vector<CarModel> car_meshes, NFSVer nfs_version, std::string car_name) {
    ...
    vehicleMotionState =
        new btDefaultMotionState(btTransform(btQuaternion(Utils::glmToBullet(car_body_model.orientation)),
                ...
                for (auto &car_model : loaded_car_models) {
                    if (car_model.m_name == "High Main Body Part") {
                        ...
                        right_rear_wheel_model = car_model;
                               ...
                               std::string::npos) { // Everything with "High" in the name is an extra body part, enable
                                                    // it
                        car_model.enable();
                        misc_models.emplace_back(car_model);
                    } else {
...
void Car::update() {
    ...
    car_body_model.position = Utils::bulletToGlm(trans.getOrigin()) +
                              (car_body_model.initialPosition * glm::inverse(Utils::bulletToGlm(trans.getRotation())));
...
void Car::genRaycasts(btDynamicsWorld *dynamicsWorld) {
    ...
    btCollisionWorld::ClosestRayResultCallback ForwardRayCallback(Utils::glmToBullet(carBodyPosition),
                                                                  Utils::glmToBullet(forwardCastPosition));
    btCollisionWorld::ClosestRayResultCallback UpRayCallback(Utils::glmToBullet(carBodyPosition),
                                                             Utils::glmToBullet(upCastPosition));
    btCollisionWorld::ClosestRayResultCallback RightRayCallback(Utils::glmToBullet(carBodyPosition),
                                                                Utils::glmToBullet(rightCastPosition));
    btCollisionWorld::ClosestRayResultCallback LeftRayCallback(Utils::glmToBullet(carBodyPosition),
                                                               Utils::glmToBullet(leftCastPosition));
    ...
    dynamicsWorld->rayTest(Utils::glmToBullet(carBodyPosition), Utils::glmToBullet(forwardCastPosition * castDistance),
                           ForwardRayCallback);
    dynamicsWorld->rayTest(Utils::glmToBullet(carBodyPosition), Utils::glmToBullet(upCastPosition * castDistance),
                           UpRayCallback);
    dynamicsWorld->rayTest(Utils::glmToBullet(carBodyPosition), Utils::glmToBullet(rightCastPosition * castDistance),
                           RightRayCallback);
    dynamicsWorld->rayTest(Utils::glmToBullet(carBodyPosition), Utils::glmToBullet(leftCastPosition * castDistance),
                           LeftRayCallback);

    if (ForwardRayCallback.hasHit()) {
        forwardDistance = glm::distance(carBodyPosition, glm::vec3(ForwardRayCallback.m_hitPointWorld.getX(),
                                                                   ForwardRayCallback.m_hitPointWorld.getY(),
                                                                   ForwardRayCallback.m_hitPointWorld.getZ()));
    } else {
        forwardDistance = castDistance;
    }
    if (UpRayCallback.hasHit()) {
        upDistance = glm::distance(carBodyPosition, glm::vec3(ForwardRayCallback.m_hitPointWorld.getX(),
                                                              ForwardRayCallback.m_hitPointWorld.getY(),
                                                              ForwardRayCallback.m_hitPointWorld.getZ()));
    } else {
    ...
    if (RightRayCallback.hasHit()) {
        rightDistance = glm::distance(carBodyPosition, glm::vec3(RightRayCallback.m_hitPointWorld.getX(),
                                                                 RightRayCallback.m_hitPointWorld.getY(),
                                                                 RightRayCallback.m_hitPointWorld.getZ()));
    } else {
        rightDistance = castDistance;
    }
    if (LeftRayCallback.hasHit()) {
        leftDistance = glm::distance(carBodyPosition, glm::vec3(LeftRayCallback.m_hitPointWorld.getX(),
                                                                LeftRayCallback.m_hitPointWorld.getY(),
                                                                LeftRayCallback.m_hitPointWorld.getZ()));
    } else {
...
void Car::applySteeringRight(bool apply) { steerRight = apply; }

void Car::applySteeringLeft(bool apply) { steerLeft = apply; }
