...
class TrainingGround {
  public:
    explicit TrainingGround(uint16_t populationSize, uint16_t nGenerations, uint32_t nTicks,
  ...
  private:
    ...
    std::vector<std::vector<int>> TrainAgents(uint16_t nGenerations,
                                              uint32_t nTicks); // Train the agents, returning agent fitness data
