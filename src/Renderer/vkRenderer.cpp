...
std::vector<char const *> const validationLayers = {"VK_LAYER_LUNARG_standard_validation"};
...
void DestroyDebugReportCallbackEXT(VkInstance instance, VkDebugReportCallbackEXT callback, VkAllocationCallbacks const *pAllocator) {
...
void vkRenderer::run() {
    {
        ...
        for (int model_Idx = 0; model_Idx < car->data.meshes.size(); ++model_Idx) {
            ...
            for (glm::vec3 vertex : car->data.meshes[model_Idx].m_vertices) {
                ...
                temp.pos = vertex;
                model_vertices[model_Idx].emplace_back(temp);
            }
        }
        initWindow();
        initVulkan();
...
void vkRenderer::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "vkNFS3", nullptr, nullptr);
}
...
void vkRenderer::createVertexBuffers() {
    for (int model_Idx = 0; model_Idx < model_vertices.size(); ++model_Idx) {
        ...
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                     vertexBuffers[model_Idx], vertexBufferMemory[model_Idx]);
...
void vkRenderer::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer,
...
VkShaderModule vkRenderer::createShaderModule(std::vector<char> const &code) {
...
VkSurfaceFormatKHR vkRenderer::chooseSwapSurfaceFormat(std::vector<VkSurfaceFormatKHR> const &availableFormats) {
...
VkPresentModeKHR vkRenderer::chooseSwapPresentMode(std::vector<VkPresentModeKHR> const availablePresentModes) {
...
VkExtent2D vkRenderer::chooseSwapExtent(VkSurfaceCapabilitiesKHR const &capabilities) {
...
std::vector<char const *> vkRenderer::getRequiredExtensions() {
