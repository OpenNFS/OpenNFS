...
namespace OpenNFS {
DebugRenderer::DebugRenderer(std::shared_ptr<BulletDebugDrawer> const &bulletDebugDrawer)
    : m_bulletDebugDrawer(bulletDebugDrawer) {
}
    ...
    for (uint32_t vroadIdx = 0; vroadIdx < nVroad; ++vroadIdx) {
        ...
        if (vroadIdx < nVroad - 1) {
            ...
            if (Config::get().useFullVroad) {
                ...
                m_bulletDebugDrawer->drawLine(Utils::glmToBullet(vroadPoint),
                                              Utils::glmToBullet(vroadPoint + curVroadRightVec), btVector3(1, 0, 0.5f));
                m_bulletDebugDrawer->drawLine(Utils::glmToBullet(vroadPoint),
                                              Utils::glmToBullet(vroadPoint - curVroadRightVec), btVector3(1, 0, 0.5f));
            }
        }
...
void DebugRenderer::DrawCameraAnimation(Track const &track) const {
    ...
    for (size_t canIdx = 0; canIdx < track.cameraAnimation.size() - 1; ++canIdx) {
        ...
        m_bulletDebugDrawer->drawLine(Utils::glmToBullet(vroadPoint + track.trackBlocks[0].position),
                                      Utils::glmToBullet(vroadPointNext + track.trackBlocks[0].position),
                                      btVector3(0, 1, 1));

        // Draw Rotations (probably interpreted incorrectly)
        glm::quat RotationMatrix{glm::normalize(
            glm::quat(refPt.od1 / 65536.0f, refPt.od2 / 65536.0f, refPt.od3 / 65536.0f, refPt.od4 / 65536.0f) *
