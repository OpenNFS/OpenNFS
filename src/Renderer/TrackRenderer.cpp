...
TrackRenderer::TrackRenderer(const shared_ptr<ONFSTrack> &activeTrack) { track = activeTrack; }

void TrackRenderer::renderTrack(const Camera &mainCamera, const Light &cameraLight,
    ...
    for (int activeBlk_Idx = 0; activeBlk_Idx < activeTrackBlockIDs.size(); ++activeBlk_Idx) {
        ...
        // TODO: Merge lighting contributions across track block, must use a smarter Track structure, also must be a
        // better way of building this from the Entities. This will be too slow.
        ...
        // TODO: Render Lanes with a simpler shader set, straight vert MVP transform w/ one texture sample on bound lane
        // texture
    ...
    for (auto &global_object : track->global_objects) {
        if (track->tag == NFS_4 || track->tag == NFS_3) {
            ...
            NFS3_4_DATA::XOBJDATA animObject = boost::get<shared_ptr<NFS3_4_DATA::TRACK>>(track->trackData)
                                                   ->xobj[globalObjIdx]
                                                   .obj[global_object.entityID];
            ...
            std::vector<GEOM_REF_BLOCK> colStructureRefData =
                track->tag == NFS_3_PS1
                    ? boost::get<shared_ptr<NFS2_DATA::PS1::TRACK>>(track->trackData)->colStructureRefData
                    : boost::get<shared_ptr<NFS2_DATA::PC::TRACK>>(track->trackData)->colStructureRefData;
                    ...
                    if (structure.recType == 3) {
                        if (animMap[global_object.entityID] < structure.animLength) {
                            boost::get<Track>(global_object.glMesh).position =
                                glm::normalize(glm::quat(glm::vec3(glm::radians(-90.f), 0, 0))) *
                                glm::vec3(
                                    structure.animationData[animMap[global_object.entityID]].position.x / 1000000.0f,
                                    structure.animationData[animMap[global_object.entityID]].position.y / 1000000.0f,
                                    structure.animationData[animMap[global_object.entityID]].position.z / 1000000.0f);
                            boost::get<Track>(global_object.glMesh).orientation =
                                glm::normalize(glm::quat(glm::vec3(glm::radians(-180.f), 0, 0))) *
                                glm::normalize(
                                    glm::quat(-structure.animationData[animMap[global_object.entityID]].unknown[0],
