...
GLuint Texture::MakeTextureArray(std::map<uint32_t, Texture> &textures, bool repeatable)
{
    ...
    for (auto &texture : textures)
    {
        ...
        glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, hsStockTextureIndexRemap(texture.first), static_cast<GLsizei>(max_width), static_cast<GLsizei>(max_height), 1, GL_RGBA, GL_UNSIGNED_BYTE,
                        &clear_data[0]);
