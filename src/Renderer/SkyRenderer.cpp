...
SkyRenderer::SkyRenderer(const shared_ptr<ONFSTrack> &activeTrack) {
    ...
    std::string err;
    std::string warn;
    ASSERT(tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, "../resources/misc/skydome/sphere.obj", nullptr,
                            true, true),
           err);
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            ...
            for (size_t v = 0; v < fv; v++) {
                ...
                verts.emplace_back(glm::vec3(attrib.vertices[3 * idx.vertex_index + 0] * 400,
...
void SkyRenderer::renderSky(const Camera &mainCamera, const Light &sun, const ParamData &userParams,
                            float elapsedTime) {
    skydomeShader.use();
    skydomeShader.loadTextures(clouds1TextureID, clouds2TextureID, sunTextureID, moonTextureID, tintTextureID,
                               tint2TextureID);
    skydomeShader.loadStarRotationMatrix(
        glm::toMat4(glm::normalize(glm::quat(glm::vec3(SIMD_PI, SIMD_PI, 0))))); // No star rotation
