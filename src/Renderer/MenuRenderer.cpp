...
namespace OpenNFS {
    MenuRenderer::MenuRenderer() {
        ...
        for (GLubyte c = 0; c < 128; c++) {
            ...
            Character character = {
                textureID, glm::ivec2(face->glyph->bitmap.width, face->glyph->bitmap.rows),
    ...
    void MenuRenderer::Render() {
        ...
        RenderText("OpenNFS v" + ONFS_VERSION + " Pre Alpha", 0, Config::get().resX - 270, 35, 0.2f,
                   glm::vec3(0.6, 0.6, 0.6));
        RenderResource("onfsLogo", 0, Config::get().resX - 75, 5, 0.1f);
        glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
    }

    void MenuRenderer::RenderText(const std::string &text, GLint layer, GLfloat x, GLfloat y, GLfloat scale,
                                  glm::vec3 colour) {
    ...
    void MenuRenderer::RenderResource(const std::string &resourceID, GLint layer, GLfloat x, GLfloat y, GLfloat scale) {
        CHECK_F(m_menuResourceMap.contains(resourceID), "Requested resourceID %s not present in menu resource map",
                resourceID.c_str());
        ...
        RenderResource(resourceID, layer, x, y, ratioX * m_menuResourceMap[resourceID].width,
                       ratioY * m_menuResourceMap[resourceID].height, scale);
    }

    void MenuRenderer::RenderResource(const std::string &resourceID, GLint layer, GLfloat x, GLfloat y, GLfloat width,
                                      GLfloat height, GLfloat scale) {
        CHECK_F(layer >= 0 && layer <= 200, "Layer: %d is outside of range 0-200", layer);
        CHECK_F(m_menuResourceMap.contains(resourceID), "Requested resourceID %s not present in menu resource map",
                resourceID.c_str());
