...
namespace OpenNFS {
    GLTexture::GLTexture(LibOpenNFS::TrackTextureAsset texture, std::vector<uint8_t> &data) : texture_asset(
            std::move(texture)), data(data) {
    }
    ...
    GLTexture GLTexture::LoadTexture(NFSVersion const tag, LibOpenNFS::TrackTextureAsset &trackTextureAsset) {
        ...
        switch (tag) {
            case NFSVersion::NFS_3: {
                if (!ImageLoader::LoadBmpWithAlpha(trackTextureAsset.fileReference.c_str(),
                                                   trackTextureAsset.alphaFileReference.c_str(), data, &width,
                                                   &height)) {
                    LOG(WARNING) << "Texture " << trackTextureAsset.fileReference << " or " << trackTextureAsset.
alphaFileReference << " did not load succesfully!";
                    // If the texture is missing, load a "MISSING" texture of identical size.
                    CHECK_F(
                        ImageLoader::LoadBmpWithAlpha("../resources/misc/missing.bmp", "../resources/misc/missing-a.bmp"
                            , data, &width, &height),
            ...
            case NFSVersion::NFS_3_PS1: {
                ...
                CHECK_F(
                    ImageLoader::LoadBmpCustomAlpha(trackTextureAsset.fileReference.c_str(), data, &width, &height,
                        alphaColour),
    ...
    GLuint GLTexture::MakeTextureArray(std::map<uint32_t, GLTexture> &textures, bool const repeatable) {
        CHECK_F(textures.size() < MAX_TEXTURE_ARRAY_SIZE,
                "Configured maximum texture array size of %d has been exceeded", MAX_TEXTURE_ARRAY_SIZE);
        ...
        LOG(INFO) << "Creating texture array with " << (int) textures.size() << " textures, max texture width " <<
 max_width << ", max texture height " << max_height;
        ...
        for (auto &[id, glTexture]: textures) {
            CHECK_F(glTexture.texture_asset.width <= max_width,
                    "Texture %u exceeds maximum specified texture width (%zu) for Array", glTexture.texture_asset.id,
                    max_width);
            CHECK_F(glTexture.texture_asset.width <= max_width,
                    "Texture %u exceeds maximum specified texture height (%zu) for Array", glTexture.texture_asset.id,
                    max_height);
    ...
    int32_t GLTexture::hsStockTextureIndexRemap(int32_t const textureIndex) {
        ...
        CHECK_F(remappedIndex <= MAX_TEXTURE_ARRAY_SIZE,
                "Texture index still exceeds max texture array size, post-remap");
