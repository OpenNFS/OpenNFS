...
std::vector<CarModel> NFS2<PC>::LoadGEO(const std::string &geo_path, std::map<unsigned int, Texture> car_textures,
    ...
    glm::quat rotationMatrix = glm::normalize(glm::quat(glm::vec3(
        0, 0, 0))); // All Vertices are stored so that the model is rotated 90 degs on X. Remove this at Vert load time.
    ...
    while (true) {
        ...
        ASSERT(geoBlockHeader->pad0 == 0 && geoBlockHeader->pad1 == 1 && geoBlockHeader->pad2 == 1,
               "Corrupt GEO block header");
        ...
        if (((end - start) % 4)) {
            LOG(DEBUG) << "Part " << part_Idx << " [" << PC_PART_NAMES[part_Idx]
                       << "] Polygon Table Pre-Pad Contents: ";
        ...
        for (uint32_t vert_Idx = 0; vert_Idx < geoBlockHeader->nVerts; ++vert_Idx) {
            verts.emplace_back(rotationMatrix * glm::vec3(vertices[vert_Idx].x / carScaleFactor,
                                                          vertices[vert_Idx].y / carScaleFactor,
                                                          vertices[vert_Idx].z / carScaleFactor));
        }
        ...
        glm::vec3 center = glm::vec3((geoBlockHeader->position[0] / 256.f) / carScaleFactor,
                                     (geoBlockHeader->position[1] / 256.f) / carScaleFactor,
                                     (geoBlockHeader->position[2] / 256.f) / carScaleFactor);
        car_meshes.emplace_back(CarModel(PC_PART_NAMES[part_Idx], verts, uvs, texture_indices, norms, indices, center,
...
std::vector<CarModel> NFS2<PS1>::LoadGEO(const std::string &geo_path, std::map<unsigned int, Texture> car_textures,
                                         std::map<std::string, uint32_t> remapped_texture_ids) {
    std::string PS1_PART_NAMES[33]{"High Additional Body Part",
                                   "High Main Body Part",
                                   "High Ground Part",
                                   "High Front Part",
                                   "High Rear Part",
    ...
    while (true) {
        ...
        for (uint32_t poly_Idx = 0; poly_Idx < geoBlockHeader->nPolygons; ++poly_Idx) {
            ...
            // TODO: There's another set of indices at index [2], that form barely valid polygons. Middle set [1] are
            // always numbers that match, 0000, 1111, 2222, 3333.
            ...
            std::vector<glm::vec2> transformedUVs =
                TrackUtils::nfsUvGenerate(NFS_3_PS1, CAR, polygons->texMap[0], gl_texture);
            uvs.insert(uvs.end(), transformedUVs.begin(), transformedUVs.end());

            glm::vec3 normal =
                rotationMatrix * calculateQuadNormal(pointToVec(vertices[polygons[poly_Idx].vertex[0][0]]),
        ...
        car_meshes.emplace_back(CarModel(PS1_PART_NAMES[part_Idx], verts, uvs, texture_indices, texMapStuff, norms,
                                         indices, center, specularDamper, specularReflectivity, envReflectivity));
...
template <typename Platform> std::shared_ptr<Car> NFS2<Platform>::LoadCar(const std::string &car_base_path) {
    ...
    // For every file in here that's a BMP, load the data into a Texture object. This lets us easily access textures by
    // an ID.
    ...
    for (boost::filesystem::directory_iterator itr(car_out_path.str()); itr != boost::filesystem::directory_iterator();
         ++itr) {
        if (itr->path().filename().string().find("BMP") != std::string::npos &&
            ...
            if (std::is_same<Platform, PS1>::value) {
                ...
                GLsizei width;
                GLsizei height;
                ASSERT(Utils::LoadBmpCustomAlpha(itr->path().string().c_str(), &data, &width, &height, 0u),
                ...
                car_textures[remapped_texture_ids[itr->path().filename().replace_extension("").string()]] =
                    Texture(remapped_texture_ids[itr->path().filename().replace_extension("").string()], data,
                            static_cast<unsigned int>(width), static_cast<unsigned int>(height));
            } else {
                bmpread_t bmpAttr; // This will leak.
                ASSERT(bmpread(itr->path().string().c_str(), BMPREAD_ANY_SIZE | BMPREAD_ALPHA, &bmpAttr),
                       "Texture " << itr->path().string() << " did not load succesfully!");
                // bmpAttr = TrackUtils::RemapNFS2CarColours(bmpAttr);
                car_textures[remapped_texture_ids[itr->path().filename().replace_extension("").string()]] =
                    Texture(remapped_texture_ids[itr->path().filename().replace_extension("").string()], bmpAttr.data,
                            static_cast<unsigned int>(bmpAttr.width), static_cast<unsigned int>(bmpAttr.height));
            }
        }
    }
    ...
    return std::make_shared<Car>(LoadGEO(geo_path.str(), car_textures, remapped_texture_ids),
                                 std::is_same<Platform, PS1>::value ? NFS_3_PS1 : NFS_2, car_name, texture_array_id);
}
...
shared_ptr<typename Platform::TRACK> NFS2<Platform>::LoadTrack(const std::string &track_base_path) {
    ...
    if (std::is_same<Platform, PC>::value) {
        ...
        ASSERT(
            TrackUtils::LoadCAN(can_path.str(), track->cameraAnimation),
    ...
    ASSERT(LoadCOL(col_path.str(), track),
           "Could not load COL file: "
               << col_path.str()); // Load Catalogue file to get global (non trkblock specific) data
    ASSERT(TrackUtils::ExtractTrackTextures(track_base_path, track->name, nfs_version),
           "Could not extract " << track->name << " texture pack.");

    // Load up the textures
    for (uint32_t tex_Idx = 0; tex_Idx < track->nTextures; tex_Idx++) {
        track->textures[track->polyToQFStexTable[tex_Idx].texNumber] =
            LoadTexture(track->polyToQFStexTable[tex_Idx], track->name, nfs_version);
    }
...
bool NFS2<Platform>::LoadTRK(std::string trk_path, const shared_ptr<typename Platform::TRACK> &track) {
    ...
    // Unknown header data
    if (trk.read(((char *)unknownHeader), sizeof(uint32_t) * 5).gcount() != sizeof(uint32_t) * 5)
        return false;
    ...
    track->superblocks =
        static_cast<typename Platform::SUPERBLOCK *>(calloc(track->nBlocks, sizeof(typename Platform::SUPERBLOCK)));
    ...
    for (uint32_t superBlock_Idx = 0; superBlock_Idx < track->nSuperBlocks; ++superBlock_Idx) {
        ...
        if (superblock->nBlocks != 0) {
            ...
            superblock->trackBlocks = static_cast<typename Platform::TRKBLOCK *>(
                calloc(static_cast<size_t>(superblock->nBlocks), sizeof(typename Platform::TRKBLOCK)));
            ...
            for (uint32_t block_Idx = 0; block_Idx < superblock->nBlocks; ++block_Idx) {
                ...
                LOG(DEBUG) << "  Block " << block_Idx + 1 << " of " << superblock->nBlocks << " ["
                           << trackblock->header->blockSerial << "]";
                ...
                trackblock->polygonTable = static_cast<typename Platform::POLYGONDATA *>(
                    calloc(static_cast<size_t>(trackblock->header->nLowResPoly + trackblock->header->nMedResPoly +
                                               trackblock->header->nHighResPoly),
                           sizeof(typename Platform::POLYGONDATA)));
                for (int32_t poly_Idx = 0;
                     poly_Idx < (trackblock->header->nLowResPoly + trackblock->header->nMedResPoly +
                                 trackblock->header->nHighResPoly);
                     ++poly_Idx) {
                ...
                trk.seekg(superblockOffsets[superBlock_Idx] + blockOffsets[block_Idx] + 64u +
                              trackblock->header->extraBlockTblOffset,
                          ios_base::beg);
                ...
                for (uint32_t xblock_Idx = 0; xblock_Idx < trackblock->header->nExtraBlocks; ++xblock_Idx) {
                    trk.seekg(superblockOffsets[superBlock_Idx] + blockOffsets[block_Idx] +
                                  extrablockOffsets[xblock_Idx],
                              ios_base::beg);
                    ...
                    switch (xblockHeader->XBID) {
                        case 5:
                            trackblock->polyTypes =
                                static_cast<POLY_TYPE *>(calloc(xblockHeader->nRecords, sizeof(POLY_TYPE)));
                        ...
                        case 4:
                            ...
                            break;
                        case 8:
                            trackblock->structures = static_cast<typename Platform::GEOM_BLOCK *>(
                                calloc(xblockHeader->nRecords, sizeof(typename Platform::GEOM_BLOCK)));
                            ...
                            for (uint32_t structure_Idx = 0; structure_Idx < trackblock->nStructures; ++structure_Idx) {
                                ...
                                trackblock->structures[structure_Idx].vertexTable =
                                    static_cast<typename Platform::VERT *>(calloc(
                                        trackblock->structures[structure_Idx].nVerts, sizeof(typename Platform::VERT)));
                                for (uint32_t vert_Idx = 0; vert_Idx < trackblock->structures[structure_Idx].nVerts;
                                     ++vert_Idx) {
                                    trk.read((char *)&trackblock->structures[structure_Idx].vertexTable[vert_Idx],
                                             sizeof(typename Platform::VERT));
                                }
                                trackblock->structures[structure_Idx].polygonTable =
                                    static_cast<typename Platform::POLYGONDATA *>(
                                        calloc(trackblock->structures[structure_Idx].nPoly,
                                               sizeof(typename Platform::POLYGONDATA)));
                                for (uint32_t poly_Idx = 0; poly_Idx < trackblock->structures[structure_Idx].nPoly;
                                     ++poly_Idx) {
                        ...
                        case 19:
                            ...
                            for (uint32_t structureRef_Idx = 0; structureRef_Idx < trackblock->nStructureReferences;
                                 ++structureRef_Idx) {
                                streamoff padCheck = trk.tellg();
                                ...
                                if (structure.recType == 1) {
                                    ...
                                    structure.animationData =
                                        static_cast<ANIM_POS *>(calloc(structure.animLength, sizeof(ANIM_POS)));
                                    for (uint32_t animation_Idx = 0; animation_Idx < structure.animLength;
                                         ++animation_Idx) {
                                        trk.read((char *)&structure.animationData[animation_Idx], sizeof(ANIM_POS));
                                    }
                                } else if (structure.recType == 4) {
                                    // 4 Component PSX Vert data? TODO: Restructure to allow the 4th component to be
                                    // read
                                    trk.read((char *)&structure.refCoordinates, sizeof(VERT_HIGHP));
                                } else {
                                    LOG(DEBUG) << "XBID " << xblockHeader->XBID
                                               << " Unknown Structure Reference type: " << (int)structure.recType
                                               << " Size: " << (int)structure.recSize
                                               << " StructRef: " << (int)structure.structureRef;
                                    continue;
                                }
                                trackblock->structureRefData.emplace_back(structure);
                                trk.seekg(structure.recSize - (trk.tellg() - padCheck),
                                          ios_base::cur); // Eat possible padding
                            }
                            break;
                            ...
                            /*case 10: {
                                ...
                                // TODO: Likely these are not VERTS, and the act of adding the parent block center gives
                            meaning where none is present. ps1TrackBlock->unknownVerts = new
                            PS1::VERT[xblockHeader->nRecords]; for (uint32_t record_Idx = 0; record_Idx <
                            xblockHeader->nRecords; ++record_Idx) { trk.read((char *)
                            &ps1TrackBlock->unknownVerts[record_Idx], sizeof(PS1::VERT));
                                }
                            }
                                break;*/
                        case 6:
                            trackblock->medianData =
                                static_cast<MEDIAN_BLOCK *>(calloc(xblockHeader->nRecords, sizeof(MEDIAN_BLOCK)));
                            trk.read((char *)trackblock->medianData, xblockHeader->nRecords * sizeof(MEDIAN_BLOCK));
                            break;
                        case 13:
                            trackblock->nVroad = xblockHeader->nRecords;
                            trackblock->vroadData = static_cast<typename Platform::VROAD *>(
                                calloc(xblockHeader->nRecords, sizeof(typename Platform::VROAD)));
                        ...
                        case 9:
                            ...
                            trackblock->laneData =
                                static_cast<LANE_BLOCK *>(calloc(xblockHeader->nRecords, sizeof(LANE_BLOCK)));
                            trk.read((char *)trackblock->laneData, trackblock->nLanes * sizeof(LANE_BLOCK));
                            break;
                        default:
                            LOG(WARNING) << "Unknown XBID: " << xblockHeader->XBID
                                         << " nRecords: " << xblockHeader->nRecords
                                         << " RecSize: " << xblockHeader->recSize;
                            break;
                    }
                    free(xblockHeader);
                }
...
bool NFS2<Platform>::LoadCOL(std::string col_path, const shared_ptr<typename Platform::TRACK> &track) {
    ...
    // Check we're in a valid TRK file
    unsigned char header[4];
    if (col.read(((char *)header), sizeof(unsigned char) * 4).gcount() != sizeof(unsigned char) * 4)
        return false;
    ...
    for (uint32_t xBlock_Idx = 0; xBlock_Idx < nExtraBlocks; ++xBlock_Idx) {
        ...
        switch (xblockHeader->XBID) {
            case 2: // First xbock always texture table
                ...
                track->polyToQFStexTable =
                    static_cast<TEXTURE_BLOCK *>(calloc(track->nTextures, sizeof(TEXTURE_BLOCK)));
            ...
            case 8: // XBID 8 3D Structure data: This block is only present if nExtraBlocks != 2
                ...
                track->colStructures = static_cast<typename Platform::GEOM_BLOCK *>(
                    calloc(track->nColStructures, sizeof(typename Platform::GEOM_BLOCK)));
                for (uint32_t structure_Idx = 0; structure_Idx < track->nColStructures; ++structure_Idx) {
                    ...
                    track->colStructures[structure_Idx].vertexTable = static_cast<typename Platform::VERT *>(
                        calloc(track->colStructures[structure_Idx].nVerts, sizeof(typename Platform::VERT)));
                    ...
                    track->colStructures[structure_Idx].polygonTable = static_cast<typename Platform::POLYGONDATA *>(
                        calloc(track->colStructures[structure_Idx].nPoly, sizeof(typename Platform::POLYGONDATA)));
            ...
            case 7: // XBID 7 3D Structure Reference: This block is only present if nExtraBlocks != 2
                ...
                for (uint32_t structureRef_Idx = 0; structureRef_Idx < track->nColStructures; ++structureRef_Idx) {
                    ...
                    if (structure.recType == 1) {
                        ...
                        LOG(WARNING) << "XBID " << xblockHeader->XBID
                                     << " Unknown COL Structure Reference type: " << (int)structure.recType
                                     << " Size: " << (int)structure.recSize
                                     << " StructRef: " << (int)structure.structureRef;
                    }
            ...
            case 15:
                ...
                track->collisionData =
                    static_cast<COLLISION_BLOCK *>(calloc(track->nCollisionData, sizeof(COLLISION_BLOCK)));
    ...
    ASSERT(col.tellg() == colSize,
           "Missing Data from the COL file! Read bytes: " << col.tellg() << " Col Reported bytes: " << colSize);
    col.close();
    return true;
}
    ...
    for (uint32_t superBlock_Idx = 0; superBlock_Idx < track->nSuperBlocks; ++superBlock_Idx) {
        ...
        for (uint32_t block_Idx = 0; block_Idx < superblock->nBlocks; ++block_Idx) {
            ...
            for (int32_t i = 0; i < trkBlock.header->nStickToNextVerts + trkBlock.header->nHighResVert; i++) {
                if (i < trkBlock.header->nStickToNextVerts) {
                    ...
                    blockReferenceCoord = (trkBlock.header->blockSerial == track->nBlocks - 1)
                                              ? track->blockReferenceCoords[0]
                                              : track->blockReferenceCoords[trkBlock.header->blockSerial + 1];
                } else {
            ...
            for (int32_t poly_Idx = (trkBlock.header->nLowResPoly + trkBlock.header->nMedResPoly);
                 poly_Idx <
                 (trkBlock.header->nLowResPoly + trkBlock.header->nMedResPoly + trkBlock.header->nHighResPoly);
                 ++poly_Idx) {
            ...
            for (uint32_t structure_Idx = 0; structure_Idx < trkBlock.nStructures; ++structure_Idx) {
                ...
                if (structureReferenceCoordinates == &track->blockReferenceCoords[trkBlock.header->blockSerial]) {
                    LOG(WARNING) << "Couldn't find a reference coordinate for Structure " << structure_Idx << " in SB"
                                 << superBlock_Idx << "TB" << block_Idx;
                }
            ...
            if (track->colStructureRefData[structRef_Idx].structureRef == structure_Idx) {
                if (track->colStructureRefData[structRef_Idx].recType == 1 ||
                    ...
                    if (track->colStructureRefData[structure_Idx].animLength != 0) {
                        // For now, if animated, use position 0 of animation sequence
                        structureReferenceCoordinates =
                            &track->colStructureRefData[structure_Idx].animationData[0].position;
                        break;
                    }
                }
            }
...
template <typename Platform> void NFS2<Platform>::ParseTRKModels(const shared_ptr<typename Platform::TRACK> &track) {
    ...
    for (uint32_t superBlock_Idx = 0; superBlock_Idx < track->nSuperBlocks; ++superBlock_Idx) {
        ...
        for (uint32_t block_Idx = 0; block_Idx < superblock->nBlocks; ++block_Idx) {
            ...
            TrackBlock current_track_block(
            ...
            if (trkBlock.nStructures != trkBlock.nStructureReferences) {
                LOG(WARNING) << "Trk block " << (int)trkBlock.header->blockSerial << " is missing "
                             << trkBlock.nStructures - trkBlock.nStructureReferences << " structure locations!";
            }
            ...
            if (std::is_same<Platform, PS1>::value) {
                ...
                for (uint32_t j = 0; j < ps1TrackBlock->nUnknownVerts; j++) {
                    glm::vec3 light_center =
                        rotationMatrix *
                        glm::vec3((refCoord->x + (256 * ps1TrackBlock->unknownVerts[j].x)) / scaleFactor,
                                  (refCoord->y + (256 * ps1TrackBlock->unknownVerts[j].y)) / scaleFactor,
                                  (refCoord->z + (256 * ps1TrackBlock->unknownVerts[j].y)) / scaleFactor);
                    current_track_block.lights.emplace_back(
                        Entity(0, j, std::is_same<Platform, PS1>::value ? NFS_3_PS1 : NFS_2, LIGHT,
                               TrackUtils::MakeLight(light_center, 0)));
                }
            }
            ...
            for (uint32_t structure_Idx = 0; structure_Idx < trkBlock.nStructures; ++structure_Idx) {
                ...
                for (uint32_t poly_Idx = 0; poly_Idx < trkBlock.structures[structure_Idx].nPoly; ++poly_Idx) {
                    ...
                    TEXTURE_BLOCK texture_for_block =
                        track->polyToQFStexTable[trkBlock.structures[structure_Idx].polygonTable[poly_Idx].texture];
                    ...
                    std::vector<glm::vec2> transformedUVs =
                        TrackUtils::nfsUvGenerate(std::is_same<Platform, PS1>::value ? NFS_3_PS1 : NFS_2, XOBJ,
                ...
                current_track_block.objects.emplace_back(
                    Entity(superBlock_Idx, (trkBlock.header->blockSerial * trkBlock.nStructures) * structure_Idx,
                           std::is_same<Platform, PS1>::value ? NFS_3_PS1 : NFS_2, XOBJ,
                           Track(verts, norms, uvs, texture_indices, vertex_indices, shading_verts, debug_data,
                                 glm::vec3(0, 0, 0))));
            }
            ...
            for (int32_t poly_Idx = (trkBlock.header->nLowResPoly + trkBlock.header->nMedResPoly);
                ...
                std::vector<glm::vec2> transformedUVs =
                    TrackUtils::nfsUvGenerate(std::is_same<Platform, PS1>::value ? NFS_3_PS1 : NFS_2, ROAD,
                                              texture_for_block.alignmentData, gl_texture);
                uvs.insert(uvs.end(), transformedUVs.begin(), transformedUVs.end());
            ...
            current_track_block.track.emplace_back(
                Entity(superBlock_Idx, trkBlock.header->blockSerial,
                       std::is_same<Platform, PS1>::value ? NFS_3_PS1 : NFS_2, ROAD,
                       Track(verts, norms, uvs, texture_indices, vertex_indices, trk_block_shading_verts, debug_data,
                             glm::vec3(0, 0, 0))));
...
std::vector<Entity> NFS2<Platform>::ParseCOLModels(const shared_ptr<typename Platform::TRACK> &track) {
    ...
    for (uint32_t structure_Idx = 0; structure_Idx < track->nColStructures; ++structure_Idx) {
        ...
        if (!structureReferenceCoordinates->x || !structureReferenceCoordinates->y ||
            !structureReferenceCoordinates->z) {
        ...
        for (uint32_t poly_Idx = 0; poly_Idx < track->colStructures[structure_Idx].nPoly; ++poly_Idx) {
            // Remap the COL TextureID's using the COL texture block (XBID2)
            TEXTURE_BLOCK texture_for_block =
                track->polyToQFStexTable[track->colStructures[structure_Idx].polygonTable[poly_Idx].texture];
            Texture gl_texture = track->textures[texture_for_block.texNumber];
        ...
        glm::vec3 position = rotationMatrix * glm::vec3(structureReferenceCoordinates->x / scaleFactor,
                                                        structureReferenceCoordinates->y / scaleFactor,
                                                        structureReferenceCoordinates->z / scaleFactor);
        col_entities.emplace_back(Entity(0, structure_Idx, NFS_2, GLOBAL,
                                         Track(verts, uvs, texture_indices, indices, shading_data, position)));
        // free(structureReferenceCoordinates);
    }
    return col_entities;
}
