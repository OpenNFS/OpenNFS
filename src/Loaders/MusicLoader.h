...
class MusicLoader {
  public:
    explicit MusicLoader(std::string const &song_base_path);
    void ParseMAP(std::string const &map_path, std::string const &mus_path);
  ...
  private:
    ...
    static void ParsePTHeader(FILE *file, uint32_t *dwSampleRate, uint32_t *dwChannels, uint32_t *dwCompression, uint32_t *dwNumSamples,
