...
void MusicLoader::ParsePTHeader(FILE *file, uint32_t *dwSampleRate, uint32_t *dwChannels, uint32_t *dwCompression,
                                uint32_t *dwNumSamples, uint32_t *dwDataStart, uint32_t *dwLoopOffset,
                                uint32_t *dwLoopLength, uint32_t *dwBytesPerSample, uint32_t *bSplit,
...
bool MusicLoader::ReadSCHl(FILE *mus_file, uint32_t sch1Offset, FILE *pcm_file) {
    ...
    ParsePTHeader(mus_file, &dwSampleRate, &dwChannels, &dwCompression, &dwNumSamples, &dwDataStart, &dwLoopOffset,
                  &dwLoopLength, &dwBytesPerSample, &bSplit, &bSplitCompression);
...
void MusicLoader::ParseMAP(const std::string &map_path, const std::string &mus_path) {
    ...
    MAPSectionDef *sectionDefTable =
        static_cast<MAPSectionDef *>(calloc(mapHeader->bNumSections, sizeof(MAPSectionDef)));
    ...
    for (auto lol = 0; lol < mapHeader->bNumSections; ++lol) {
        if (!ReadSCHl(mus_file, startingPositions[lol], pcm_file)) { //
            std::cout << "Error reading SCHl block, POS: " << (int)lol << " Offset: " << startingPositions[lol]
                      << std::endl;
            break;
        }
    }
    ...
    while (sectionDefTable[section_Idx].bNumRecords > 0 && section_Idx < mapHeader->bNumSections) {
        ...
        if (!ReadSCHl(mus_file, startingPositions[section_Idx], pcm_file)) { //
            std::cout << "Error reading SCHl block, POS: " << (int)section_Idx
                      << " Offset: " << startingPositions[section_Idx] << std::endl;
            break;
        }

        // Check if we've already played this section before. If we have, drop record number, else set to highest record
        // index
        playedSections[section_Idx] = playedSections.count(section_Idx) ? playedSections[section_Idx] - 1
                                                                        : sectionDefTable[section_Idx].bNumRecords - 1;
