...
std::shared_ptr<TRACK> NFS4::LoadTrack(const std::string &track_base_path) {
    ...
    ASSERT(ExtractTrackTextures(track_base_path, track->name, NFSVer::NFS_4),
...
std::vector<CarModel> NFS4::LoadFCE(const std::string &fce_path) {
    ...
    for (uint32_t part_Idx = 0; part_Idx < fceHeader->nParts; ++part_Idx) {
        ...
        glm::vec3 center =
            rotationMatrix * glm::vec3(fceHeader->partCoords[part_Idx].x / 10, fceHeader->partCoords[part_Idx].y / 10,
        ...
        fce.seekg(sizeof(FCE::NFS4::HEADER) + fceHeader->vertTblOffset +
                      (fceHeader->partFirstVertIndices[part_Idx] * sizeof(FLOATPT)),
                  ios_base::beg);
        ...
        for (uint32_t vert_Idx = 0; vert_Idx < fceHeader->partNumVertices[part_Idx]; ++vert_Idx) {
            vertices.emplace_back(rotationMatrix * glm::vec3(partVertices[vert_Idx].x / 10,
                                                             partVertices[vert_Idx].y / 10,
        ...
        fce.seekg(sizeof(FCE::NFS4::HEADER) + fceHeader->triTblOffset +
                      (fceHeader->partFirstTriIndices[part_Idx] * sizeof(FCE::TRIANGLE)),
                  ios_base::beg);
        ...
        for (uint32_t tri_Idx = 0; tri_Idx < fceHeader->partNumTriangles[part_Idx]; ++tri_Idx) {
            ...
            if (isTraffic) {
                ...
                uvs.emplace_back(
                    glm::vec2(partTriangles[tri_Idx].uvTable[0], 1.0f - partTriangles[tri_Idx].uvTable[3]));
                uvs.emplace_back(
                    glm::vec2(partTriangles[tri_Idx].uvTable[1], 1.0f - partTriangles[tri_Idx].uvTable[4]));
                uvs.emplace_back(
                    glm::vec2(partTriangles[tri_Idx].uvTable[2], 1.0f - partTriangles[tri_Idx].uvTable[5]));
            }
        }
...
bool NFS4::LoadFRD(const std::string &frd_path, const std::string &track_name, const std::shared_ptr<TRACK> &track) {
            ...
            if (track->xobj[j].nobj > 0) {
                ...
                for (k = 0; k < track->xobj[j].nobj; k++) {
                    ...
                    /*				if (x->crosstype==6) { // object with byte data
                                                            ...
                                                            if
                       ((long)ar.read((char*)x->hs_type6,x->unknown2)!=x->unknown2) return false;
                                                    }
                    */
    ...
    if (track->xobj[j].nobj > 0) {
        ...
        for (uint32_t k = 0; k < track->xobj[j].nobj; k++) {
            ...
            /*				if (x->crosstype==6) { // object with byte data
                                                    x->hs_type6=(char *)malloc(x->unknown2);
                                                    if ((long)ar.read((char*)x->hs_type6,x->unknown2)!=x->unknown2)
               return false;
                                            }
            */
            ar.read((char *)x->vert, 12 * x->nVertices);
    ...
    uint64_t nData = std::count_if(directory_iterator(tex_path), directory_iterator(),
...
std::vector<TrackBlock> NFS4::ParseTRKModels(const std::shared_ptr<TRACK> &track) {
    ...
    for (uint32_t i = 0; i < track->nBlocks; i++) {
        ...
        TrackBlock current_track_block(i,
                                       rotationMatrix * glm::vec3(trk_block.ptCentre.x / 10, trk_block.ptCentre.y / 10,
        ...
        for (uint32_t j = 0; j < trk_block.nLightsrc; j++) {
            ...
            current_track_block.lights.emplace_back(
                Entity(i, j, NFS_4, LIGHT, MakeLight(light_center, trk_block.lightsrc[j].type)));
        }
        ...
        for (uint32_t s = 0; s < trk_block.nSoundsrc; s++) {
            ...
            current_track_block.sounds.emplace_back(
                Entity(i, s, NFS_4, SOUND, Sound(sound_center, trk_block.soundsrc[s].type)));
        }
            ...
            if (obj_polygon_block.n1 > 0) {
                ...
                for (uint32_t k = 0; k < obj_polygon_block.nobj; k++) {
                    ...
                    for (uint32_t p = 0; p < obj_polygon_block.numpoly[k]; p++) {
                        ...
                        glm::vec3 normal =
                            rotationMatrix * calculateQuadNormal(pointToVec(trk_block.vert[object_polys[p].vertex[0]]),
                        ...
                        std::vector<glm::vec2> transformedUVs =
                            nfsUvGenerate(NFS_4, OBJ_POLY, object_polys[p].hs_texflags, gl_texture, texture_for_block);
                    ...
                    current_track_block.objects.emplace_back(
                        Entity(i, (j + 1) * (k + 1), NFS_4, OBJ_POLY,
                               Track(obj_verts, norms, uvs, texture_indices, vertex_indices, obj_shading_verts,
                                     trk_block_center)));
                }
            }
        }
        ...
        for (uint32_t l = (i * 4); l < (i * 4) + 4; l++) {
            for (uint32_t j = 0; j < track->xobj[l].nobj; j++) {
                ...
                for (uint32_t k = 0; k < x->nPolygons; k++, x->polyData++) {
                    ...
                    std::vector<glm::vec2> transformedUVs =
                        nfsUvGenerate(NFS_4, XOBJ, x->polyData->hs_texflags, gl_texture, texture_for_block);
                    uvs.insert(uvs.end(), transformedUVs.begin(), transformedUVs.end());
                ...
                current_track_block.objects.emplace_back(Entity(
                    i, l, NFS_4, XOBJ,
                    Track(verts, norms, uvs, texture_indices, vertex_indices, xobj_shading_verts, trk_block_center)));
            }
        }
        ...
        for (uint32_t chnk = 4; chnk <= 6; chnk++) {
            ...
            for (uint32_t k = 0; k < polygon_block.sz[chnk]; k++) {
                ...
                glm::vec3 normal =
                    rotationMatrix * calculateQuadNormal(pointToVec(trk_block.vert[poly_chunk[k].vertex[0]]),
                ...
                std::vector<glm::vec2> transformedUVs = nfsUvGenerate(
                    NFS_4, chnk == 6 ? LANE : ROAD, poly_chunk[k].hs_texflags, gl_texture, texture_for_block);
                uvs.insert(uvs.end(), transformedUVs.begin(), transformedUVs.end());
            ...
            if (chnk == 6) {
                current_track_block.lanes.emplace_back(Entity(i, -1, NFS_4, LANE,
                                                              Track(verts, norms, uvs, texture_indices, vertex_indices,
                                                                    trk_block_shading_verts, trk_block_center)));
            } else {
                current_track_block.track.emplace_back(Entity(i, -1, NFS_4, ROAD,
                                                              Track(verts, norms, uvs, texture_indices, vertex_indices,
                                                                    trk_block_shading_verts, trk_block_center)));
            }
        }
    ...
    for (uint32_t j = 0; j < track->xobj[globalObjIdx].nobj; j++) {
        ...
        glm::vec3 position = rotationMatrix * glm::vec3(static_cast<float>(x->ptRef.x / 65536.0) / 10,
                                                        static_cast<float>(x->ptRef.y / 65536.0) / 10,
                                                        static_cast<float>(x->ptRef.z / 65536.0) / 10);
        track->global_objects.emplace_back(
            Entity(-1, j, NFS_4, GLOBAL,
                   Track(verts, norms, uvs, texture_indices, vertex_indices, xobj_shading_verts, position)));
    }
...
Texture NFS4::LoadTexture(TEXTUREBLOCK track_texture, const std::string &track_name) {
    ...
    if (track_texture.islane) {
        ...
        filename_alpha << "../resources/sfx/" << setfill('0') << setw(4) << (track_texture.texture - 2048) + 9
                       << "-a.BMP";
    } else {
        filename << TRACK_PATH << ToString(NFS_4) << "/" << track_name << "/textures/" << setfill('0') << setw(4)
                 << track_texture.texture << ".BMP";
        filename_alpha << TRACK_PATH << ToString(NFS_4) << "/" << track_name << "/textures/" << setfill('0') << setw(4)
    ...
    if (!Utils::LoadBmpCustomAlpha(filename.str().c_str(), &data, &width, &height, 0)) {
        ...
        ASSERT(Utils::LoadBmpWithAlpha("../resources/misc/missing.bmp", "../resources/misc/missing-a.bmp", &data,
                                       &width, &height),
