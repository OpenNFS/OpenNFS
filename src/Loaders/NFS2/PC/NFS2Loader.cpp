...
constexpr char PS1_PART_NAMES[33][32]{"High Additional Body Part",
                                      "High Main Body Part",
                                      "High Ground Part",
                                      "High Front Part",
                                      "High Rear Part",
		...
		if (superblock->nBlocks != 0)
		{
			...
			for (uint32_t block_Idx = 0; block_Idx < superblock->nBlocks; ++block_Idx)
			{
				...
				trackblock->polygonTable = static_cast<typename Platform::POLYGONDATA *>(
				  calloc(static_cast<size_t>(trackblock->header->nLowResPoly + trackblock->header->nMedResPoly + trackblock->header->nHighResPoly),
				...
				for (uint32_t xblock_Idx = 0; xblock_Idx < trackblock->header->nExtraBlocks; ++xblock_Idx)
				{
					...
					case 19:
						...
						for (uint32_t structureRef_Idx = 0; structureRef_Idx < trackblock->nStructureReferences; ++structureRef_Idx)
						{
							...
							{
								LOG(DEBUG) << "XBID " << xblockHeader->XBID << " Unknown Structure Reference type: " << (int) structure.recType
								           << " Size: " << (int) structure.recSize << " StructRef: " << (int) structure.structureRef;
								continue;
							}
	...
	for (uint32_t xBlock_Idx = 0; xBlock_Idx < nExtraBlocks; ++xBlock_Idx)
	{
		...
		case 8: // XBID 8 3D Structure data: This block is only present if nExtraBlocks != 2
			...
			for (uint32_t structure_Idx = 0; structure_Idx < track->nColStructures; ++structure_Idx)
			{
				...
				track->colStructures[structure_Idx].vertexTable =
				  static_cast<typename Platform::VERT *>(calloc(track->colStructures[structure_Idx].nVerts, sizeof(typename Platform::VERT)));
				...
				track->colStructures[structure_Idx].polygonTable =
				  static_cast<typename Platform::POLYGONDATA *>(calloc(track->colStructures[structure_Idx].nPoly, sizeof(typename Platform::POLYGONDATA)));
		...
		case 7: // XBID 7 3D Structure Reference: This block is only present if nExtraBlocks != 2
			...
			for (uint32_t structureRef_Idx = 0; structureRef_Idx < track->nColStructures; ++structureRef_Idx)
			{
				...
				{
					LOG(WARNING) << "XBID " << xblockHeader->XBID << " Unknown COL Structure Reference type: " << (int) structure.recType << " Size: " << (int) structure.recSize
	...
	for (uint32_t superBlock_Idx = 0; superBlock_Idx < track->nSuperBlocks; ++superBlock_Idx)
	{
		...
		for (uint32_t block_Idx = 0; block_Idx < superblock->nBlocks; ++block_Idx)
		{
			...
			for (int32_t poly_Idx = (trkBlock.header->nLowResPoly + trkBlock.header->nMedResPoly);
			     poly_Idx < (trkBlock.header->nLowResPoly + trkBlock.header->nMedResPoly + trkBlock.header->nHighResPoly); ++poly_Idx)
			{
				obj_dump << "f " << (unsigned int) trkBlock.polygonTable[poly_Idx].vertex[0] + 1 << " " << (unsigned int) trkBlock.polygonTable[poly_Idx].vertex[1] + 1 << " "
				         << (unsigned int) trkBlock.polygonTable[poly_Idx].vertex[2] + 1 << " " << (unsigned int) trkBlock.polygonTable[poly_Idx].vertex[3] + 1 << std::endl;
			}
			obj_dump.close();
	...
	for (uint32_t structure_Idx = 0; structure_Idx < track->nColStructures; ++structure_Idx)
	{
		...
		for (uint32_t poly_Idx = 0; poly_Idx < track->colStructures[structure_Idx].nPoly; ++poly_Idx)
		{
			obj_dump << "f " << (unsigned int) track->colStructures[structure_Idx].polygonTable[poly_Idx].vertex[0] + 1 << " "
			         ...
			         << (unsigned int) track->colStructures[structure_Idx].polygonTable[poly_Idx].vertex[3] + 1 << std::endl;
		}
		obj_dump.close();
	...
	for (uint32_t superBlock_Idx = 0; superBlock_Idx < track->nSuperBlocks; ++superBlock_Idx)
	{
		...
		for (uint32_t block_Idx = 0; block_Idx < superblock->nBlocks; ++block_Idx)
		{
			...
			if (std::is_same<Platform, PS1>::value)
			{
				...
				{
					glm::vec3 light_center = rotationMatrix * glm::vec3((refCoord->x + (256 * ps1TrackBlock->unknownVerts[j].x)) / scaleFactor,
