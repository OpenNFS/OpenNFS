    ...
    if (nfs_version == NFS_3_PS1) {
        ...
        if (boost::filesystem::exists(sky_fsh_path.str())) {
            ...
            ASSERT(Utils::ExtractQFS(sky_fsh_path.str(), sky_textures_path.str()),
                   "Unable to extract sky textures from " << sky_fsh_path.str());
        }
    }
...
GLuint MakeTextureArray(std::map<unsigned int, Texture> &textures, bool repeatable) {
    ASSERT(textures.size() < MAX_TEXTURE_ARRAY_SIZE,
           "Configured maximum texture array size of " << MAX_TEXTURE_ARRAY_SIZE << " has been exceeded.");
    ...
    // Find the maximum width and height, so we can avoid overestimating with blanket values (256x256) and thereby scale
    // UV's uneccesarily
    ...
    glTexStorage3D(GL_TEXTURE_2D_ARRAY, 3, GL_RGBA8, max_width, max_height,
                   MAX_TEXTURE_ARRAY_SIZE); // I should really call this on textures.size(), but the layer numbers are
                                            // not linear up to textures.size(). HS Bloats tex index up over 2048.

    for (auto &texture : textures) {
        ASSERT(texture.second.width <= max_width, "Texture " << texture.second.texture_id
                                                             << " exceeds maximum specified texture size (" << max_width
                                                             << ") for Array");
        ASSERT(texture.second.height <= max_height, "Texture " << texture.second.texture_id
                                                               << " exceeds maximum specified texture size ("
                                                               << max_height << ") for Array");
        // Set the whole texture to transparent (so min/mag filters don't find bad data off the edge of the actual image
        // data)
        glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, hsStockTextureIndexRemap(texture.first), max_width, max_height, 1,
                        GL_RGBA, GL_UNSIGNED_BYTE, &clear_data[0]);
        glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, hsStockTextureIndexRemap(texture.first), texture.second.width,
                        texture.second.height, 1, GL_RGBA, GL_UNSIGNED_BYTE,
                        (const GLvoid *)texture.second.texture_data);

        texture.second.min_u = 0.00;
        texture.second.min_v = 0.00;
        texture.second.layer = hsStockTextureIndexRemap(texture.first);
        texture.second.max_u = (texture.second.width / static_cast<float>(max_width)) -
                               0.005f; // Attempt to remove potential for sampling texture from transparent area
...
std::vector<glm::vec2> nfsUvGenerate(NFSVer tag, EntityType mesh_type, uint32_t textureFlags, Texture gl_texture) {
    ...
    switch (tag) {
        ...
        case NFS_2_SE:
            switch (mesh_type) {
                case XOBJ: {
                    ...
                    glm::mat2 uvRotationTransform = glm::mat2(cos(glm::radians(angle)), sin(glm::radians(angle)),
                                                              -sin(glm::radians(angle)), cos(glm::radians(angle)));
                    uvs.emplace_back(((glm::vec2(1.0f, 1.0f) - originTransform) * uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(0.0f, 1.0f) - originTransform) * uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(0.0f, 0.0f) - originTransform) * uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(1.0f, 1.0f) - originTransform) * uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(0.0f, 0.0f) - originTransform) * uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(1.0f, 0.0f) - originTransform) * uvRotationTransform) +
                                     originTransform);
                    for (auto &uv : uvs) {
                        if (horizontalFlip) {
                            uv.x = 1.0f - uv.x;
        ...
        case NFS_4:
            ASSERT(false,
                   "Incorrect UV generate function call, did you mean to pass in an NFS3_4::TEXTUREBLOCK as well?");
            break;
        case NFS_5:
            ASSERT(false, "Unimplemented");
            break;
...
std::vector<glm::vec2> nfsUvGenerate(NFSVer tag, EntityType mesh_type, uint32_t textureFlags, Texture gl_texture,
    ...
    switch (tag) {
        ...
        case NFS_3:
            switch (mesh_type) {
                case XOBJ:
                    uvs.emplace_back((1.0f - texture_block.corners[0]) * gl_texture.max_u,
                                     (1.0f - texture_block.corners[1]) * gl_texture.max_v);
                    uvs.emplace_back((1.0f - texture_block.corners[2]) * gl_texture.max_u,
                                     (1.0f - texture_block.corners[3]) * gl_texture.max_v);
                    uvs.emplace_back((1.0f - texture_block.corners[4]) * gl_texture.max_u,
                                     (1.0f - texture_block.corners[5]) * gl_texture.max_v);
                    uvs.emplace_back((1.0f - texture_block.corners[0]) * gl_texture.max_u,
                                     (1.0f - texture_block.corners[1]) * gl_texture.max_v);
                    uvs.emplace_back((1.0f - texture_block.corners[4]) * gl_texture.max_u,
                                     (1.0f - texture_block.corners[5]) * gl_texture.max_v);
                    uvs.emplace_back((1.0f - texture_block.corners[6]) * gl_texture.max_u,
                                     (1.0f - texture_block.corners[7]) * gl_texture.max_v);
                    break;
                case OBJ_POLY:
                case LANE:
                case ROAD:
                    uvs.emplace_back(texture_block.corners[0] * gl_texture.max_u,
                                     (1.0f - texture_block.corners[1]) * gl_texture.max_v);
                    uvs.emplace_back(texture_block.corners[2] * gl_texture.max_u,
                                     (1.0f - texture_block.corners[3]) * gl_texture.max_v);
                    uvs.emplace_back(texture_block.corners[4] * gl_texture.max_u,
                                     (1.0f - texture_block.corners[5]) * gl_texture.max_v);
                    uvs.emplace_back(texture_block.corners[0] * gl_texture.max_u,
                                     (1.0f - texture_block.corners[1]) * gl_texture.max_v);
                    uvs.emplace_back(texture_block.corners[4] * gl_texture.max_u,
                                     (1.0f - texture_block.corners[5]) * gl_texture.max_v);
                    uvs.emplace_back(texture_block.corners[6] * gl_texture.max_u,
                                     (1.0f - texture_block.corners[7]) * gl_texture.max_v);
                    break;
                case GLOBAL:
                    break;
        ...
        case NFS_4:
            switch (mesh_type) {
                ...
                case XOBJ: {
                    ...
                    glm::mat2 uvRotationTransform = glm::mat2(cos(glm::radians(angle)), sin(glm::radians(angle)),
                                                              -sin(glm::radians(angle)), cos(glm::radians(angle)));
                    uvs.emplace_back(((glm::vec2(1.0f - texture_block.corners[0], 1.0f - texture_block.corners[1]) -
                                       originTransform) *
                                      uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(1.0f - texture_block.corners[2], 1.0f - texture_block.corners[3]) -
                                       originTransform) *
                                      uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(1.0f - texture_block.corners[4], 1.0f - texture_block.corners[5]) -
                                       originTransform) *
                                      uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(1.0f - texture_block.corners[0], 1.0f - texture_block.corners[1]) -
                    ...
                    uvs.emplace_back(((glm::vec2(1.0f - texture_block.corners[4], 1.0f - texture_block.corners[5]) -
                                      ...
                                      uvRotationTransform) +
                                     originTransform);
                    uvs.emplace_back(((glm::vec2(1.0f - texture_block.corners[6], 1.0f - texture_block.corners[7]) -
                                       originTransform) *
                                      uvRotationTransform) +
                                     originTransform);
                    for (auto &uv : uvs) {
                ...
                case GLOBAL: {
                    ...
                    glm::mat2 uvRotationTransform = glm::mat2(cos(glm::radians(angle)), sin(glm::radians(angle)),
                                                              -sin(glm::radians(angle)), cos(glm::radians(angle)));
                    uvs.emplace_back(
                        ((glm::vec2(texture_block.corners[0], 1.0f - texture_block.corners[1]) - originTransform) *
                         uvRotationTransform) +
                        originTransform);
                    uvs.emplace_back(
                        ((glm::vec2(texture_block.corners[2], 1.0f - texture_block.corners[3]) - originTransform) *
                         uvRotationTransform) +
                        originTransform);
                    uvs.emplace_back(
                        ((glm::vec2(texture_block.corners[4], 1.0f - texture_block.corners[5]) - originTransform) *
                         uvRotationTransform) +
                        originTransform);
                    uvs.emplace_back(
                        ((glm::vec2(texture_block.corners[0], 1.0f - texture_block.corners[1]) - originTransform) *
                    ...
                    uvs.emplace_back(
                        ...
                        originTransform);
                    uvs.emplace_back(
                        ((glm::vec2(texture_block.corners[6], 1.0f - texture_block.corners[7]) - originTransform) *
                         uvRotationTransform) +
                        originTransform);
                    for (auto &uv : uvs) {
                        if (horizontalFlip) {
...
bool LoadCAN(std::string can_path, std::vector<SHARED::CANPT> &cameraAnimations) {
    ...
    for (uint8_t anim_Idx = 0; anim_Idx < animLength; ++anim_Idx) {
        ...
        cameraAnimations[anim_Idx].od2 = cameraAnimations[anim_Idx].od2; // OD2 seems to be something more than just
    rotation, like zoom or some shit cameraAnimations[anim_Idx].od3 = cameraAnimations[anim_Idx].od3; // OD3 seems to
    set perspective cameraAnimations[anim_Idx].od4 = cameraAnimations[anim_Idx].od4; // OD4 similar to OD1, induces
    wavyness but animation remains can_out.write((char *) &cameraAnimations[anim_Idx], sizeof(SHARED::CANPT));
    }
    can_out.close();*/
