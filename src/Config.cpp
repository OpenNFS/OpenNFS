...
namespace OpenNFS {
    ...
    void Config::InitFromCommandLine(int argc, char **argv) {
        ...
        program.add_argument("--vulkan").help("Use the Vulkan renderer instead of GL default").flag().store_into(
            vulkanRender);
        program.add_argument("--headless").help("Launch ONFS without a window").flag().store_into(headless);
        program.add_argument("--nracers", "-r").help("Number of AI Racers to spawn").default_value(DEFAULT_NUM_RACERS).
                store_into(nRacers);
        ...
        program.add_argument("--carv", "-cv")
                ...
                .store_into(carTag);
        program.add_argument("--track", "-t").help("Name of desired track").default_value(DEFAULT_TRACK).
                store_into(track);
        program.add_argument("--trackv", "-tv")
                ...
                .store_into(trackTag);
        program.add_argument("--resX", "-x").help("Horizontal screen resolution").default_value(DEFAULT_X_RESOLUTION).
                store_into(resX);
        program.add_argument("--resY", "-y").help("Vertical screen resolution").default_value(DEFAULT_Y_RESOLUTION).
                store_into(resY);
        program.add_argument("--fixup-asset-paths")
                .help(
                    "Rename all available NFS files and folders to lowercase so can be consistent for ONFS read on Unix systems")
                .flag()
